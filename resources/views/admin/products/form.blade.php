...
<form
    ...
    method="POST" enctype="multipart/form-data" class="space-y-6 p-6 transition-all" x-data="{
        selectedSizes: {{ Js::from(
            old('sizes', isset($product) ? $product->stockCombinations->pluck('size_id')->unique()->toArray() : []),
        ) }}
    }">
    @csrf
    @if (isset($product))
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mt-2">
            @foreach ($sizes as $size)
                <label class="cursor-pointer">
                    <input type="checkbox" name="sizes[]" x-model.number="selectedSizes" value="{{ $size->id }}"
                        class="hidden" />

                    <div class="border p-3 rounded-lg transition-all"
                        :class="{
                            'border-pink-500 bg-pink-50 dark:bg-pink-900/20': selectedSizes.includes(
                                {{ $size->id }}),
                            'hover:border-pink-300 dark:hover:border-pink-600': !selectedSizes.includes(
                                {{ $size->id }})
                        }">
                        <div class="flex flex-col items-center space-y-1">
                            ...
                            <span class="text-xs text-pink-600 dark:text-pink-400"
                                x-text="selectedSizes.includes({{ $size->id }}) ? 'Selected' : 'Click to select'"></span>
                        </div>
                        ...
                        <div x-show="selectedSizes.includes({{ $size->id }})" x-transition class="mt-4">
                            <div class="grid grid-cols-2 gap-2">
                                @foreach ($colors as $color)
                                    @php
                                        $rec = isset($product)
                                            ? $product->stockCombinations->firstWhere(
                                                fn($c) => $c->size_id == $size->id && $c->color_id == $color->id,
                                            )
                                            : null;
                                    @endphp
                                    ...
                                    <div class="space-y-1 text-center">
                                        ...
                                        <input type="number" name="stocks[{{ $size->id }}-{{ $color->id }}]"
                                            min="0"
                                            ...
                                            value="{{ old('stocks.' . $size->id . '-' . $color->id, $rec?->stock ?? '') }}"
                                            placeholder="0" />
